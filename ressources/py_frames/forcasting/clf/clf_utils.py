...
@dataclass
class BestParams:
    logistic: Dict[str, Any] = field(
        default_factory=lambda: {"C": 0.1, "max_iter": 300, "solver": "lbfgs"}
    )
    ...
    extra_trees: Dict[str, Any] = field(
        default_factory=lambda: {
            "bootstrap": True,
            "max_depth": 5,
            "max_features": "log2",
    ...
    knn: Dict[str, Any] = field(
        default_factory=lambda: {"n_neighbors": 7, "p": 1, "weights": "uniform"}
    )
...
def add_targets_and_lags(df: pd.DataFrame, lag_months: list[int]) -> pd.DataFrame:
    ...
    for col in ratio_cols:
        for lag in lag_months:
            df_clf[f"{col}_lag{lag}"] = (
                df_clf.groupby("client")[col].shift(lag).fillna(0)
            )
    return df_clf
...
def split_data(df: pd.DataFrame, features: list[str], test_frac=5):
    ...
    return (
        train_df[features],
        train_df["channel_next"],
        test_df[features],
        test_df["channel_next"],
        train_df,
        test_df,
    )
...
def prepare_data(
    df,
    lag_months,
    extra_features=["code_guichet", "month_sin", "month_cos", "nb_switch_3m"],
):
    df = df.copy()
    ...
    features = [
        ...
        *df_feat.filter(regex="^chan_t_").columns.tolist(),
    ] + extra_features
...
def train_classifier(
    X_train, y_train, model_name: str = "gb", best_params: dict = None
):
    pipeline = Pipeline([("clf", CLASSIFIERS[model_name](**best_params))])
